  ...
  module.exports = function Optimizer(options, context) {
    ...
    var tokenize = function(body, selector) {
      ...
      for (var i = 0,
        ...
        keyValues.push([property, value, token.value.indexOf('!important') > -1, token.value.indexOf(IE_BACKSLASH_HACK, firstColon + 1) === token.value.length - IE_BACKSLASH_HACK.length, token.metadata]);
      }
      return keyValues;
    };
    var optimize = function(tokens, allowAdjacent) {
      ...
      tokensLoop: for (var i = 0,
        ...
        var _property = (property == '-ms-filter' || property == 'filter') ? (lastProperty == 'background' || lastProperty == 'background-image' ? lastProperty : property) : property;
    ...
    var rebuild = function(tokens) {
      ...
      for (var i = 0,
        ...
        var property = !tokens[i][0] && tokens[i][1].indexOf('__ESCAPED_') === 0 ? tokens[i][1] : tokens[i][0] + ':' + tokens[i][1];
        tokenized.push({
          value: property,
          metadata: tokens[i][4]
        });
    ...
    return {process: function(selector, body, allowAdjacent, compactProperties) {
        var tokenized = tokenize(body, selector);
        var optimized = optimize(tokenized, allowAdjacent);
        var rebuilt = rebuild(optimized);
        return shorthandCompacting && compactProperties && rebuilt.compactFurther ? compact(rebuilt.tokenized) : rebuilt;
      }};
  };
})(require("process"));
