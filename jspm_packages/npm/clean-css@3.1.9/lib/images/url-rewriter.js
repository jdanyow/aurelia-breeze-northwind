...
(function(process) {
  ...
  function rebase(resource, options) {
    ...
    var specialUrl = resource[0] == '/' || resource[0] == '#' || (!options.imports && importUrl) || dataUri || /^https?:\/\//.exec(resource) !== null || /__\w+__/.exec(resource) !== null;
    var rebased;
    ...
    if (options.absolute) {
      rebased = path.resolve(path.join(options.fromBase, resource)).replace(options.toBase, '');
    } else {
      rebased = path.relative(options.toBase, path.join(options.fromBase, resource));
    }
    return process.platform == 'win32' ? rebased.replace(/\\/g, '/') : rebased;
  }
